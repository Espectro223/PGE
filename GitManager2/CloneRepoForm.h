...
#pragma endregion
		...
		System::Void btnCloneRepo_Click(System::Object^ sender, System::EventArgs^ e) {
			...
			try {
				...
				// Mostrar la salida o el error
				if (!String::IsNullOrEmpty(errorOutput)) {
					this->rtbCloneOutput->AppendText("Error: " + errorOutput + "\n");
				}
				else {
					this->rtbCloneOutput->AppendText("Repositorio clonado exitosamente en " + folderName + "\n");
				}
			}
			catch (Exception^ ex) {
